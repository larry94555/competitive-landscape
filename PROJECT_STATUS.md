...
| **S3** | **Ready for use** — friendly users should find no issue | **No.** | [**89%**](docs/Full_Feature_List.md#s3--ready-for-use) | 6 of 9 report sections, no verification layer, no comparison matrix, no accounts. **A finished report does hand its whole self to a reader's own assistant** — Run 43 — **and a run in progress now says how far through it is**, with a percentage that is pages read rather than a guess — Run 46. |
