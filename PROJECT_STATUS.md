...
**As of 2026-08-14** · `main` at `2b624e1`, plus the branch this page is on.
...
**And the wait now says how long is left.** A reader deploying the product asked why the page
...
[PRODUCT_IDEA_RESULTS.md](docs/PRODUCT_IDEA_RESULTS.md) specifies what the first screen shows
and [`prototype/results-mockup.html`](prototype/results-mockup.html) shows it; the page is now
both. **Every sentence on it that makes a claim about provenance reads it from the report**
rather than guessing — a reader who typed three domains is told *"You named 3 companies"*, not
that we found them, and a count taken over a search that did not finish says *"at least"*. See
[BENCHMARKS.md](docs/BENCHMARKS.md) Run 47.
...
| ~~**B2**~~ | ~~**The search channel reaches nothing.**~~ **Closed.** Queries, the provider seam, a SearXNG adapter, admission, candidate generation, and set derivation — and an analysis that starts from a description and ends in a comparison. | ~~F1 R6/R7/R8, F2, F3, F8's matrix, S2~~ | Agent | Four PRs, from *"does not exist"* to a description producing a set. **What it does not close is [B1](#4-blockers)**: nothing measures whether the set is right, and a channel nobody can score is a channel nobody can improve. **SearXNG has now been run against this.** It was true for six phases that no engine had ever answered this application a query; one has, on the deployed instance, and [BENCHMARKS.md](docs/BENCHMARKS.md) Run 50 is the first entry in that file with a real engine behind it. **What it produced was a bad answer** — *"project management for a small design agency"* returned Microsoft and a keyword-shaped domain — from four independent causes in the discovery logic, none of which any test could see. That is [B1](#4-blockers), arriving exactly where B1 said it would, and it is now a counted row: [`IMPROVING_PRODUCT_IDEAS_LOGIC_ROADMAP.md`](docs/IMPROVING_PRODUCT_IDEAS_LOGIC_ROADMAP.md), **seven pull requests, all seven done.** **B1 is now partly answered, for discovery.** Five fixtures score an idea against a known-correct set of companies with no engine configured, and the number is **mean recall 60% with one impostor admitted** — [BENCHMARKS.md](docs/BENCHMARKS.md) Run 51. It is a bad number on purpose: it is what the code does today, and the five changes left in that roadmap are each scored against it. What is still open in B1 is the other half — nothing scores the *report* about the companies once they are found. **And two of the four causes are fixed.** A candidate is a product rather than a domain, read from the page a query returned, so the reported failure leads with *Asana* and calls Microsoft *Microsoft Project* on the two queries Project earned rather than *Microsoft* on three — [BENCHMARKS.md](docs/BENCHMARKS.md) Run 52. And the fit test is no longer one shared word: it asks for half the market's words, chosen by scoring four rules against the golden set rather than by picking a number, and the board game that shared *project* with the prompt is gone — Run 53. **The reported answer is now Asana and Microsoft Project**, with no impostor. **And the market's own buyer's guides are read rather than discarded** — they were on `NOT_A_COMPANY`, never candidates and never evidence either, and what a guide *links to* is what it says is in the market. Two independent guides naming a company is corroboration a single search cannot give, which is what puts `workamajig.com` — the specialist the prompt actually asked for — into the answer. **Nothing is left of that reader's complaint:** the fixtures return Asana, Workamajig, Microsoft Project and Notion Projects, with the board game named as excluded and why — Run 54. **And the argument for the set is now on the page**: each company carries the sentence that put it there, and every company the searching found and the set refused is named with its reason — which was the last thing this product asserted without showing its evidence — Run 55. **And a question the market's own guides say has several answers is now offered rather than answered** — the categories come from the guides' headings, `Failure::TooGeneral` is its own kind rather than being told to the reader as *we could not work out which company you meant*, and each chip carries a whole prompt — Run 56. **The plan is complete.** What it did not do is measure that last rule: the discovery fixtures are markets with one answer each, so nothing in the golden set exercises it, and the concentration threshold is a hypothesis with nine unit tests behind it. That is written down in Run 56 rather than left for somebody to discover. |
...
| **B5** | **Partly closed. An instance is serving.** The units are running, the per-address cap is enforcing, `SEARX_URL` is set and the search channel answers — so [DEPLOY.md](docs/DEPLOY.md) has been walked far enough to produce reports. **What is still unconfirmed from here** is the domain, HTTPS, the instance shape and Pay-As-You-Go status, and no latency figure has been taken from a client's side. Nothing in this row is verifiable by the agent; it is reported from what the founder has run. | Phase 0's latency exit criterion, and every number that can only be taken from a client's side. **No longer any readiness state**: those are defined by what the software does, verified locally. | Founder | Claude stops at the PR. The first person through DEPLOY.md is the one who finds out where it is wrong. |
...
**To S2 — any business idea handled correctly. Every software PR is done.** The search channel is finished
...
the board it names — and the set that came out of all of it is **editable**, so a reader who
disagrees with the derivation can correct it in a click rather than retype the idea, and a search
that fails says which of three things happened rather than recommending a wait for all of them.
**It has been run**, and the first real answer a search engine has given this application was a
bad one — see [BENCHMARKS.md](docs/BENCHMARKS.md) Run 50 and
[`IMPROVING_PRODUCT_IDEAS_LOGIC_ROADMAP.md`](docs/IMPROVING_PRODUCT_IDEAS_LOGIC_ROADMAP.md). **All six extractors are
...
stranger without apologizing?"* **One row of it is done ahead of the rest**: a finished report
hands its whole self to whatever assistant a reader already pays for, as Markdown with a URL and
a date against every sentence ([BENCHMARKS.md](docs/BENCHMARKS.md) Run 43). It was pulled forward
because it settles what this is — not a worse chatbot, the evidence file a chatbot cannot
assemble — and because S2's last row cannot be closed from a keyboard.
