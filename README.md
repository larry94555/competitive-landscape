...
| `cargo run -p landscape -- candidates <domain>` | The other half: who a **named** company competes with. Queries templated from the company's own name for itself, so nothing you typed reaches the engine |
...
thirty-six entries, each with the symptom a reader would have seen and the question that would
...
instead of the decision that number drives, and one where a fixture with a single element could
not test an ordering.
...
report belongs to the worker that wrote it. Every row carries a **generation** — how many times the run
has been started — which a worker quotes on every write, so a worker the sweep has replaced
