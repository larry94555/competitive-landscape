...
| [COMPETITIVE_ANALYSIS_REPORT.md](COMPETITIVE_ANALYSIS_REPORT.md) | What the report contains: the nine sections, the chart catalog, evidence classes, what is deliberately excluded, and the charting decision |
| [PRODUCT_IDEA_RESULTS.md](PRODUCT_IDEA_RESULTS.md) | **What a reader sees the moment a run finishes** — their own words, how the idea was interpreted, one count, and three lists. Owns the first screen, demotes the claim sections to a detail view and a PDF, and states which of the four blocks cannot be built yet and why. **Built** — [BENCHMARKS.md](BENCHMARKS.md) Run 47; §6 records where the page departs from the plan and what is still outstanding |
...
| [DEPLOY.md](DEPLOY.md) | The same deployment as an argument: why the ports are open, why the artifacts stay root-owned, why both inference artifacts are pinned, and which two steps are guesses — and the measurement that can only be taken from the client's side |
...
≤900-token generation budget, aggressive multi-layer caching (two users analyzing the same
...
**Nothing is done until it can be demonstrated end to end on a laptop.** No item below is
complete because its tests pass; it is complete when somebody sitting in front of
`cargo run -p landscape -- dev` with a `llama-server` beside it can *see* the difference.

**And deploying must change nothing but secrets.** Every readiness state in
[PROJECT_STATUS.md](../PROJECT_STATUS.md) is defined by what the software can do, verified
locally; the host is where other people get to see it. Everything environmental is already an
environment variable — `BIND_ADDR`, `DATABASE_URL`, `LLAMA_URL`, `WEB_DIR` — and if running on
the box ever needs a code change, that is a defect to fix rather than a task to schedule.
...
citations invite clicking and whether an honest gap reads as rigor. Show it to concierge
...
(the domain starts aging and the list starts growing from day one); run the **name and
...
- ~~**Quality toolchain from commit one** per [CODING_QUALITY.md](CODING_QUALITY.md): workspace
  ...
  **Retrofitting a quality bar onto an existing codebase does not work** — the gates must
  predate the code.
- ~~`docs/decisions/` initialized with the ADR template, and `docs/TUTORIAL.md` skeleton with
...
- Bake-off across **Qwen3-1.7B / 4B / 8B / 14B**, plus **Gemma 3 4B/12B** and
  **Llama 3.2 3B** as alternates, at **Q4_K_M *and* Q4_0** (ARM repacking may make the
  ...
  [ADR 0007](decisions/0007-model-licenses.md). All three families are usable; **Qwen3 is
...
- `landscape-golden` — ten frozen pages, hand-written answers, and a score. The set
  **validates itself** with no model running: every reference answer must appear verbatim on
  ...
  its own existence in prose. It scores the defective quantization at **10%** against 87%
...
- Three findings that only measurement could produce, all in [BENCHMARKS.md](BENCHMARKS.md):
  a **defective quantization produced schema-valid garbage** (shape guarantees are not
...
- ~~Drive the orchestrator through the states a run only reaches when something goes wrong.~~
  **Done** — [BENCHMARKS.md](BENCHMARKS.md) Run 18. A worker that dies, a run that is
  ...
  belong to one that no longer exists — which no server-side judgment about a *connection*
...
- **Entity resolution before any fetch**, with the disambiguation gate
  ...
  **The gate is built** — `landscape-core::subject`, pure and fully unit-tested: given scored
  ...
  the fetching this gate exists to authorize. Built in this order deliberately: written
...
- Map-reduce analysis: per-source structured extraction → section synthesis.
  ...
  *What they claim to hold* — `landscape-extract::assurance`, Run 24 — is the fifth and the
  ...
  this held or being worked toward*. linear.app names four.
  ...
  **An add-on is a heading with a price under it**, which is exactly the shape a plan has.
  ...
  ~~**Discovery admits localized duplicates.**~~ Fixed in Run 9: a locale is a variant of a
  ...
  **What remains here is measurement, not more rules.** Discovery has no golden-set subjects at
  all, so every judgment about it — these two included — rests on four companies read by hand.
...
- **Review-platform access audit** (§ week 1, before any architecture assumes the channel):
  ...
  **highest-yield discovery channel**, and the sentiment section leans on the same platforms —
  but the plan honors robots.txt as a hard ethical commitment. **These two positions may
...
- ~~`landscape-search`: `SourceProvider` trait; SearXNG adapter; trust tiering; source ranking
  ...
  **A prompt that names no domain runs now** — [BENCHMARKS.md](BENCHMARKS.md) Run 29, the second
  ...
  for, the gate decides, and one clear answer is analyzed. The other two verdicts are refusals
  ...
  **This one deviates from `PRODUCT_SPEC.md` §3 on purpose:** there is no *"skip, just analyze"*
...
- **The market decides what is searched for** — [BENCHMARKS.md](BENCHMARKS.md) Run 36, and the
  ...
  **"Did the words change" is decided on the queries, not on the strings.** `for_idea` normalizes
...
- ~~Fetch cache + per-source extraction cache~~ (the highest-leverage cache in the system —
  ...
  **And the expensive half.** The fetch cache took the second reader's fetches to zero and left
  ...
  model at the same address, and one model's words labeled with another's is the failure this
...
not care what kind of organization is being analyzed. So a failed positioning is a **relabeling
...
`user_api_keys` excluded from ordinary backups; deletion honored immediately; custom base
...
- **0 → 1** is a *quality* jump more than a speed jump: Qwen3-30B-A3B is a materially better
  grounded summarizer than an 8B, and MoE keeps it CPU-affordable. Reports get better before
  they get fast.
