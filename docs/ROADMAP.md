...
| **D6** | **A cap on anonymous runs** | Two a day per address, `PRODUCT_SPEC.md` §2.1's number, counted where a run *starts* so a reader watching one is never cut off. The address is the rightmost `X-Forwarded-For` entry — the one the proxy observed rather than the one a client claims — hashed with a per-process key. A request with no proxy in front of it is a laptop and is not counted. | **Done** |
